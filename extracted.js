...
function hidePanels() {
  ...
  document.getElementById("ragPage").style.display = "none";
  document.getElementById("langchainPage").style.display = "none";
}

function showRAG() {
  hidePanels();
  document.getElementById("ragPage").style.display = "flex";
  setActiveToc("toc-rag");
}

function showLangchain() {
  hidePanels();
  document.getElementById("langchainPage").style.display = "flex";
  // always land on overview when entering via sidebar
  document.getElementById("lc-overview").style.display = "block";
  document.getElementById("lc-demo-container").style.display = "none";
  setActiveToc("toc-langchain");
}
...
const _lcRunners = {
  ...
  1: async () => {
    ...
    const data = await lcPost('/lc/prompt', {role, question: q});
    document.getElementById('lc-right').innerHTML = `
  ...
  2: async () => {
    ...
    const data = await lcPost('/lc/chaining', {text});
  ...
  3: async () => {
    ...
    if (!q) throw new Error('Enter a question.');
    const data = await lcPost('/lc/rag', {question: q});
    document.getElementById('lc-right').innerHTML = `
  ...
  4: async () => {
    ...
    const data = await lcPost('/lc/memory', {message: msg, session_id: _getLCSession()});
  ...
  5: async () => {
    ...
    if (!q) throw new Error('Enter a question.');
    const data = await lcPost('/lc/tools', {question: q});
  ...
  6: async () => {
    ...
    if (!text) throw new Error('Enter some text.');
    const data = await lcPost('/lc/documents', {text});
  ...
  7: async () => {
    ...
    const data = await lcPost('/lc/parsers', {topic});
  ...
  8: async () => {
    ...
    if (!q) throw new Error('Enter a question.');
    const data = await lcPost('/lc/agent', {question: q});
  ...
  9: async () => {
    ...
    if (!topic) throw new Error('Enter a topic.');
    const data = await lcPost('/lc/multiagent', {topic});
    document.getElementById('lc-right').innerHTML = `
  ...
  10: async () => {
    ...
    if (!topic) throw new Error('Enter a topic.');
    const data = await lcPost('/lc/langgraph', {topic});
...
async function lcMemoryClear() {
  ...
  await fetch(`${BASE}/lc/memory/${sid}`, {method:'DELETE'}).catch(()=>{});
  _lcSession = null;
